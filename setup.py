...
class BuildPy(setuptools.command.build_py.build_py):
    ...
    """
    @staticmethod
    def sanity_check_success():
        dirname = os.path.dirname(os.path.abspath(__file__)) + '/frontend/server/build/libs'
        if os.path.isdir(dirname) and any(".jar" in file_name for file_name in os.listdir(dirname)) and \
                time.time() - os.stat(source_server_file).st_mtime < 300 and os.path.isfile(dest_file_name):
            return True
        return False

    def run(self):
        if BuildPy.sanity_check_success() is False and 'build_frontend' not in opt_set:
            sys.stderr.write('\033[91m' +
                             'Runing \"python setup.py build_frontend\" - missing or old binary found.' +
                             '\033[0m' + '\n')
            sys.stderr.flush()
